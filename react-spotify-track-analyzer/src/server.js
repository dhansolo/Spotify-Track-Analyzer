...
async function track_analyzer(accessToken) {
    ...
    try {
        ...
        for(let track of tracks.data.items) {
            ...
            if(seed_artists.length < 5 && !seed_artists.includes(track.artists[0].id)) {
                seed_artists.push(track.artists[0].id)
            }
        }
    } catch(err) {
    ...
    let recs;
    try {
        recs = await SpotifyHelper.get_song_recommendations(accessToken, seed_artists, avg_danceability, avg_energy, avg_positivity);
    } catch(err) {
